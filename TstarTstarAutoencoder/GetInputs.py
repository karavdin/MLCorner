...
def GetInputs(parameters):
    ...
    with open(inputdir+inputsubdir+systvar+'/'+ 'variable_names.pkl', 'rb') as f:
        variable_names = pickle.load(f, encoding='bytes') #conversion of string needed in python 3.x
    ...
    maxfiles_per_sample = {'TTbar': -1, 'WJets': -1, 'ST': -1, 'DYJets': -1, 'TstarTstarToTgluonTgluon_M-700': -1, 'TstarTstarToTgluonTgluon_All': -1, 'TstarTstarToTgluonTgluon_M-1500': -1}
...
#        print lists_of_inputfiles
    ...
    # now read in signal
    signal_masses = [700, 1500] #FixME: should be set in steer file
    signal_identifiers = ['TstarTstarToTgluonTgluon_All'] #FixME: should be set in steer file
    for mass in signal_masses:
        signal_identifiers.append('TstarTstarToTgluonTgluon_M-' + str(mass))
    all_signals = {}
...
def ApplyPrepocessing(parameters, outputfolder, filepostfix, setid):
    ...
    print('ApplyPrepocessing stores in path: ',outputfolder)
